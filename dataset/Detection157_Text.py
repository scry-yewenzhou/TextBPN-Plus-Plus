...
class Detection157Text(TextDataset):

    def __init__(self, data_root, load_memory=False, transform=None, ignore_list=None):
        ...
        # p = re.compile('(.jpg|.JPG|.PNG|.png|.JPEG)')
        # self.annotation_list = ['{}'.format(p.sub("", img_name)) for img_name in self.image_list]
        self.image_list = [x for x in self.image_list if 
                           x.lower().endswith(".jpg") or 
                           x.lower().endswith(".jpeg") or 
                           x.lower().endswith(".png")]
        # self.annotation_list = [f"gt_{Path(x).stem}" for x in self.image_list]
...
if __name__ == '__main__':
    ...
    trainset = Detection157Text(
        data_root='../data/detection157icdar2015',
        transform=transform
    )
